...
- 日期: 2026-07-17
- 版本: v0.4.0
- 状态: demo 可运行, v0.4.0 功能闭环全通 (新增对外只读 API + 外部 Agent 调用 skill + 站点 favicon), 回归测试 164/164 全通过, **已部署阿里云 + GitHub 同步**, 对外 API 已通过真实云端 curl 调用验证
...
力跑双训分析系统 — 力量(训记) + 跑步(Garmin) 双线训练分析 demo。整合两条线数据 + 当前目标配置 + 休整标注, 生成可存档的双线分析报告, 识别冲突/过量并给方向性建议。v0.2 新增: AI 目标澄清对话 / 报告浅追问 / 温度—心率热解读 / 首页数据看板 + 粒度切换 + 分布饼图。v0.3 新增: 单日训练分享卡片 (7 种视觉方案, 3:4 竖版, 隐私过滤, 手动截图)。v0.4 新增: 对外只读 API v1 (`/api/v1/*` 9 端点, Bearer Key 鉴权, 只读不触发 LLM) + 外部 Agent 调用 skill + 站点 favicon。首页是训记式月历看板, 可点进每天详情。
...
```bash
python tests/test_end_to_end.py      # 164 项端到端回归, 用临时库, 不污染 var/
```

覆盖: 登录/鉴权、真实 Garmin 导入(多文件+去重)、跑步类型分类、训记 seed、肌群映射(含手动订正+AI补全endpoint)、目标(含历史查看/复用/中文标签)、AI 目标澄清(无 LLM 优雅降级 + `created_by` 标记/白名单)、凭证可用性徽章、轻量 Markdown 渲染(基础语法 + XSS 转义)、休整标注、分析报告(规则引擎)、重新分析、AI 浅追问(无 LLM 优雅降级 + 落库/渲染)、热负荷-心率解读(热档分档/室内排除/心率差)、手工标注气温(设值/清空/越界/非数字)、日历看板、首页数据看板(日/周/月三档各12桶/合计校验/肌群排序/跑步类型分布)、每日详情、单日分享卡片(鉴权重定向/非法+不存在日期/七主题白名单/多跑步聚合稳定性+隐私过滤)、对外只读 API(Key 签发/鉴权/跨用户隔离/只读 405/吊销/isolation)、各页面渲染、登出。

此外于 2026-07-16 进行了真实云端 API 手工 smoke test (只读 GET, 不触发 LLM), 验证 meta/context/days 端点可用, 无凭证/假 Key 均正确返回 401。

## 4a. 安全注意事项

- API Key (`srda_` 前缀) 仅以 SHA-256 hash 存储于 `api_keys` 表, 明文**仅签发时一次性显示**, 之后不可再现。不可将 Key 写入日志、代码、聊天记录或提交。
- 当前生产为方案 A 明文 HTTP, Bearer Key 在信道上可被嗅探。**仅限所有者自用/可信网络; 对外开放或他人接入须等到方案 C (HTTPS)。**
- 销毁旧 Key 后再签发新 Key 不会影响既有平台数据, 所有 Key 均绑定同一个 user_id。
...
app/
  ...
  repositories.py    数据访问 + 业务编排 (同步/导入/日历/报告/近期训练概况/看板聚合; day_share_detail 分享白名单视图 + 多跑步稳定聚合)
  services/
  ...
  templates/         Jinja2 页面 (含 goal_clarify.html 目标澄清对话; day_share.html 单日分享卡片, 独立布局不复用侧栏)
  static/
    css/
      app.css        后台全部样式 (含轻量 Markdown 渲染 / 首页看板 / 分段控件 / 饼图)
      share.css      v0.3; 分享卡片专用, 作用域限定 .share-page/.poster, 含 7 主题 + 3:4 布局, 不污染 app.css
    js/
...
- `user_credentials` 加密凭证 / `api_keys` 平台 API Key (hash-only) / `operation_logs`
...
| 首页数据看板 | ✅ | v0.2; 日历上方训练概览: 日/周/月粒度切换(各 12 桶, 前端切换零请求); 力量容量趋势 + 跑量趋势(柱状图) + 肌群容量分布(横向条形) + 跑步类型分布(SVG 环形饼图, 按距离占比, 图例附次数); 纯只读聚合, 零第三方图表库; 周一起始与日历一致 |
...
| 对外只读 API v1 端点 | ✅ | v0.4; `/api/v1/*` 共 9 个只读 GET 端点; Bearer Key 鉴权, 按 user_id 隔离; `OUTBOUND_API_ENABLED` 门控; 仅读已有存量, 不写入/不同步/不触发 LLM |
| 对外 API Key 管理 | ✅ | v0.4; owner 在 `/settings/api-keys` 签发/吊销; `api_keys` 表存 sha256 hash; 明文仅签发时一次显示; 可多 Key/可分别吊销 |
| 外部 Agent 调用 skill | ✅ | v0.4; `skills/strength-running-readonly-api/SKILL.md` (Agent Skill) + `LLM-INSTRUCTIONS.md` (通用 LLM 指令); curl-only, 9 端点全覆盖, 含安全/约束/示例 |
| 站点 favicon | ✅ | v0.4; `app/static/favicon.svg`, 复用分享卡片品牌图标 (绿色圆角方块 + 白色折线) |
...
- ~~对外只读 API 实际端点~~ ✅ 已于 v0.4.0 实现 (`/api/v1/*` 9 端点, Bearer key 鉴权, 只读, 见 ADR 0004; `OUTBOUND_API_ENABLED` 门控)
- 邀请码注册 UI / 用户管理后台
- 手动运动记录 (预留不开发)
...
- 凭证加密存储, 不写日志, 不完整回显。
- 不做主动教练/完整训练计划/医疗建议。
- 分享卡片是隐私过滤后的独立视图: 只出训练部位/总容量与跑步汇总字段, 不得回退到 `day_detail` 全量数据或暴露动作明细/GPS/地点/分圈/海拔/气温/功率/步频/活动 ID; 新增分享字段前先确认不泄露隐私。仍是登录后私有页, 不加匿名分享 token。

## 10. 部署 (ADR 0001)

- 已部署: 阿里云轻量应用服务器 (华东, 2vCPU/2GB/40GB, Ubuntu 24.04); 当前线上版本 v0.4.0 (2026-07-16)
...
- 分享卡片入口只在「有训练的每日详情页」出现: 日历只对有数据的日期渲染 `/day/{date}` 链接, 空白日无入口。分享路由的空状态 (无训练时的极简提示) 仅在直接输入 URL 时可达, 属兜底而非常规路径。

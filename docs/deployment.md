...
- 后续维护: Claude
- 服务器版本: v0.4.0 (2026-07-17) — 对外只读 API + Agent 调用 skill + 站点 favicon; 真实云端 API 调用验证通过
...
- **v0.4.0 (2026-07-17)** — 站点 favicon (`app/static/favicon.svg`, 复用分享卡品牌图标) + Agent 调用 skill (`skills/strength-running-readonly-api/`)。云端 API 真实调用验证: 有效 Key 读 meta/context/days 均 200; 无凭证/假 Key 401; 跨用户隔离正确。favicon 公网 `/static/favicon.svg` 200, Content-Type: image/svg+xml。上线前 WAL 安全备份 (`app-20260717-012353.db.gz`, integrity=ok)。无 schema 变更, 无新 Python 依赖。
